...
// ============================================================================
// parse_json — Parse Bureau of Labor Statistics API JSON response
// ============================================================================
...
DataSet parse_json(const std::string& json_input) {
    ...
    while (obj) {
        ...
        const char* year_val = find_key(obj, obj_e, "year");
        const char* period_val = find_key(obj, obj_e, "period");
        if (year_val && period_val) {
            int y = static_cast<int>(parse_number(year_val, obj_e));
            std::string per = parse_string(period_val, obj_e);
            int m = (per.size() >= 3) ? atoi(per.c_str() + 1) : 1;
            rec.timestamp = ((y - 1970) * 365.25 + (m - 1) * 30.44) * 86400.0;
        }
        ...
        const char* val_p = find_key(obj, obj_e, "value");
        rec.value = parse_number(val_p, obj_e);

        const char* id_p = find_key(obj, obj_e, "seriesID");
        rec.source_id = parse_string(id_p, obj_e);

        const char* cat_p = find_key(obj, obj_e, "periodName");
        rec.category = parse_string(cat_p, obj_e);

        const char* desc_p = find_key(obj, obj_e, "footnotes");
...
bool validate(const std::string& input) {
    ...
    if (input.find("\"Results\"") != std::string::npos) return true;
